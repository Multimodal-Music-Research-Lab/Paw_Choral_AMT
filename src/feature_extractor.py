...
class LogMelExtractor(nn.Module):
    ...
    def __init__(self, sample_rate=44100, fft_size=1024, frames_per_second=86,
        ...
        if torchaudio is None:
            raise ImportError("torchaudio is required for LogMelExtractor but could not be imported.")
        self.mel_bins = 229
        # Alt settings:
...
class MoSQIToExtractor(nn.Module):
    ...
    def __init__(self, sample_rate=44100, mode="sone"):
        ...
        self.sample_rate = sample_rate
        self.mode = mode
        if loudness_zwtv is None:
            raise ImportError("mosqito is required for MoSQIToExtractor but is not installed.")
